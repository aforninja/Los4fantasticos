...
loadTextures({
    "layer1": "tgheroes:fantastic_4/doom/doctor_doom",
	"cape": "tgheroes:fantastic_4/doom/doctor_doom_cape",
	"lights": "tgheroes:blank",
	"ears": "tgheroes:blank",
	
	"blank": "tgheroes:blank",
    "tp": "tgheroes:fantastic_4/doom/doom_tp.tx.json",
});
...
var tp_beam; 
var aoe_charge; 
var doom_mele_aoe = implement("tgheroes:external/doom_hurt");

function init(renderer) {
    parent.init(renderer);	
	renderer.setTexture((entity, renderLayer) => {
            var timer = entity.getInterpolatedData("tgheroes:dyn/teleport");
            return timer < 0.01 ? "layer1" : timer < 0.7 && entity.getInterpolatedData("fiskheroes:teleport_timer") > 0 ? "tp" : timer < 0.7 && entity.getInterpolatedData("fiskheroes:teleport_timer") == 0 ? "tp" : "blank";
    });
...
function initEffects(renderer) {
	...
	doom_mele_aoe.init(renderer, "tgheroes:lightning_green");
...
function initAnimations(renderer) {
	...
	addAnimationWithData(renderer, "doom.ELECTRIC", "fiskheroes:dual_aiming", "fiskheroes:beam_charge");	
	addAnimationWithData(renderer, "doom.AIMING", "fiskheroes:dual_aiming", "fiskheroes:aiming");	

    addAnimationWithData(renderer, "doom.AOE", "tgheroes:electric_boom", "tgheroes:dyn/electric_timer").setCondition(entity => entity.getData("tgheroes:dyn/electric"));
    addAnimationWithData(renderer, "doom.FIELD", "tgheroes:star_pose", "fiskheroes:shield_blocking_timer").setCondition(entity => entity.getData("tgheroes:dyn/spell_heal"));
    addAnimationWithData(renderer, "doom.MAGIC_SHIELD", "fiskheroes:dual_aiming", "fiskheroes:shield_blocking_timer").setCondition(entity => entity.getData("tgheroes:dyn/spell_basic"));
...
function render(entity, renderLayer, isFirstPersonArm) {
	...
	if (entity.getWornChestplate().suitType() == "tgheroes:doctor_doom/god" || entity.getWornChestplate().suitType() == "tgheroes:doctor_doom/2099" || entity.getWornChestplate().suitType() == "tgheroes:doctor_doom" ) {
